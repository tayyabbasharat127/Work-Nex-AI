...
export default function AdminAnalytics() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      // Attendance trends: backend returns [{date, status, _count}] - need to pivot
      if (trendsData.status === 'fulfilled') {
        const raw = trendsData.value;
        const arr = Array.isArray(raw) ? raw : (raw?.data || []);
        // Pivot: group by date, spread statuses
        const map = {};
        arr.forEach(r => {
          const d = r.date ? new Date(r.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : r.date;
          if (!map[d]) map[d] = { date: d, PRESENT: 0, ABSENT: 0, LATE: 0 };
          map[d][r.status] = r._count?.status || r.count || 0;
        });
        setAttendanceTrends(Object.values(map));
      }
      ...
      if (headData.status === 'fulfilled') setHeadcount(headData.value);
    } catch (err) {
