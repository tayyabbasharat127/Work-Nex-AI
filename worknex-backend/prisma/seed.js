...
async function main() {
  ...
  for (const p of policyData) {
    const existing = await prisma.leavePolicy.findFirst({ where: { leaveType: p.leaveType } });
    const policy = existing
      ? await prisma.leavePolicy.update({ where: { id: existing.id }, data: p })
    ...
    if (s.status === 'APPROVED') {
      ...
      if (policy) {
        await prisma.leaveBalance.updateMany({
          where: { userId: emp.id, policyId: policy.id, year: YEAR },
  ...
  for (const n of notifData) {
    ...
    await prisma.notification.create({
      data: { userId: user.id, type: n.type, title: n.title, message: n.msg, isRead: n.read },
    });
  }
  ...
  for (const user of allUsers) {
    for (let month = 1; month <= 3; month++) {
      ...
      await prisma.performanceRecord.upsert({
        ...
        update: { presentDays, absentDays, lateDays, leaveDays, avgWorkingHours, attendanceScore, leaveScore, overallScore },
        create: { userId: user.id, month, year: YEAR, presentDays, absentDays, lateDays, leaveDays, avgWorkingHours, attendanceScore, leaveScore, overallScore },
      });
      perfCount++;
    }
  }
  ...
  for (const log of etlLogs) {
    await prisma.etlSyncLog.create({ data: log });
  }
