...
const registerOrganization = async (data) => {
  const { orgName, industry, country, ownerEmail, ownerFirstName, phone, website } = data;
  ...
  return { organization: org, licenseKey, trialEndsAt: trialEnd };
};
...
const checkEmployeeLimit = async (organizationId) => {
  ...
  const empCount = await prisma.user.count({ where: { isActive: true } });
