...
import { Search, Plus, Eye, Edit, X, ChevronLeft, ChevronRight, UserCheck, UserX } from 'lucide-react';
                              ...
                              <button
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    toast.success(`${user.name} has been ${isActive ? 'disabled' : 'enabled'}`);
                                  } catch {
                                    toast.error('Failed to update user status');
                                  }
                                }}
                                className={`p-2 rounded-lg transition group ${user.status === 'Active' ? 'hover:bg-destructive/10' : 'hover:bg-success/10'}`}
                                title={user.status === 'Active' ? 'Disable user' : 'Enable user'}
                              >
                                {user.status === 'Active'
                                  ? <UserX size={16} className="text-muted-foreground group-hover:text-destructive" />
                                  : <UserCheck size={16} className="text-muted-foreground group-hover:text-success" />
                                }
                              </button>
                            </div>
                          </td>
