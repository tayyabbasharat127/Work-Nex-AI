...
export default function EmployeeDashboard() {
  ...
  const [currentTime, setCurrentTime] = useState(null); // null on both server+client
  ...
  useEffect(() => {
    setMounted(true);
    // Read user from localStorage only on client
    try {
  ...
  // Show nothing until client has mounted — same render on server and client
        ...
        <div className="sticky top-0 bg-card/80 backdrop-blur-xl border-b border-border p-6 z-20">
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
            <div>
              ...
              <p className="text-muted-foreground mt-1" suppressHydrationWarning>{currentTime ? currentTime.toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : ''}</p>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="text-3xl font-bold text-primary font-mono" suppressHydrationWarning>{currentTime ? currentTime.toLocaleTimeString() : '--:--:--'}</div>
            </div>
          </div>
        </div>
